 ...
 * 
 * @author Hamid A. Toussi (hamid2c@gmail.com): Making TypeManager faster by
 *         making type masks during a depth-first-traversal on the class
 *         hierarchy. First, type-masks of the leaves of Class Hierarchy are
 *         created and then the type mask of each type T is obtained by ORing
 *         type maks of Types sub-types and setting the bit-numbers associated
 *         with Allocation Nodes of type T. The type-mask of each interface is
 *         achieved by ORing the type-masks of its top-level concrete
 *         implementers. In fact, Reference types are visited in
 *         reversed-topological-order.
 */
public final class TypeMask {
	private Map<SootClass, List<Alloc_Node>> class2allocs = new HashMap<SootClass, List<Alloc_Node>>(1024);
	private List<Alloc_Node> anySubtypeAllocs = new LinkedList<Alloc_Node>();

	public TypeMask(PAG pag) {
		this.pag = pag;
	}
	...
	final public BitVector get(Type type) {
		...
		if (ret == null && fh != null) {
			...
			if (type instanceof RefType && curClass.isPhantom())
				...
				// Scan through the hierarchy. We might have a phantom class
				// higher up
	...
	final public void makeTypeMask() {
		...
		if (PTAOptions.sparkOpts.verbose())
		...
		// **
		ArrayNumberer<Alloc_Node> allocNodes = pag.getAllocNodeNumberer();
		for (Type t : Scene.v().getTypeNumberer()) {
			...
			if (t instanceof RefType && !t.equals(RefType.v("java.lang.Object"))
					&& !t.equals(RefType.v("java.io.Serializable")) && !t.equals(RefType.v("java.lang.Cloneable"))) {
	...
	private void initClass2allocs() {
		for (Alloc_Node an : pag.getAllocNodeNumberer()) {
			addAllocNode(an);
		}
	}

	final private void addAllocNode(final Alloc_Node alloc) {
		alloc.getType().apply(new TypeSwitch() {
			final public void caseRefType(RefType t) {
				...
				if ((list = class2allocs.get(cl)) == null) {
					list = new LinkedList<Alloc_Node>();
	...
	final private BitVector makeClassTypeMask(SootClass clazz) {
		...
		List<Alloc_Node> allocs = null;
		if (clazz.isConcrete()) {
			allocs = class2allocs.get(clazz);
		}
		if (allocs != null) {
			for (Alloc_Node an : allocs) {
		...
		if (subclasses == Collections.EMPTY_LIST) {
			for (Alloc_Node an : anySubtypeAllocs) {
				mask.set(an.getNumber());
			}
