...
/**
 * Implementation of points-to set that holds two sets: one for new elements
 * that have not yet been propagated, and the other for elements that have
...
public class DoublePTSet extends PTSetInternal {
	...
	public DoublePTSet(Type type, PAG pag) {
		super(type);
	 ...
	 */
	public boolean addAll(PTSetInternal other, PTSetInternal exclude) {
		if (exclude != null) {
	...
	private static PTSetFactory defaultP2SetFactory = new PTSetFactory() {
		public PTSetInternal newSet(Type type, PAG pag) {
			return new DoublePTSet(type, pag);
		}
	};

	public static PTSetFactory getFactory(PTSetFactory newFactory, PTSetFactory oldFactory) {
